...
def try_locate(cli):
  ...
  if not (
    ...
    err_msg = 'Darknet is not compiled yet (darknet/darknet[.exe]). Please build it before using the extension (read darknet/README.md for building instructions).' if lang != 'ru_RU' else 'Darknet не скомпилирован (darknet/darknet[.exe]). Пожалуйста, соберите его, прежде чем использовать расширение (прочтите darknet/README.md для инструкций по сборке).'
    if verbose:
      print(err_msg)
    cli.send_error('[Darknet] ' + err_msg)
    return False
  if not os.path.exists(os.path.join('darknet', 'cfg', 'yolov7-tiny.cfg')):
    err_msg = 'YOLOv7 model configuration file (darknet/cfg/yolov7-tiny.cfg) not found. Please download it from <a href=\"https://github.com/AlexeyAB/darknet\">this link</a> before using the extension.' if lang != 'ru_RU' else 'Файл конфигурации модели YOLOv7 (darknet/cfg/yolov7-tiny.cfg) не найден. Пожалуйста, скачайте его по <a href=\"https://github.com/AlexeyAB/darknet\">ссылке</a>, прежде чем использовать расширение.'
    if verbose:
      print(err_msg)
    cli.send_error('[Darknet] ' + err_msg)
    return False
  if not os.path.exists(os.path.join('darknet', 'yolov7-tiny.weights')):
    err_msg = 'YOLOv7 model weights file (darknet/yolov7-tiny.weights) not found. Please download it from <a href=\"https://github.com/WongKinYiu/yolov7/releases/download/v0.1/yolov7-tiny.weights\">this link</a> before using the extension.' if lang != 'ru_RU' else 'Файл весов модели YOLOv7 (darknet/yolov7-tiny.weights) не найден. Пожалуйста, скачайте его по <a href=\"https://github.com/WongKinYiu/yolov7/releases/download/v0.1/yolov7-tiny.weights\">ссылке</a>, прежде чем использовать расширение.'
    if verbose:
      print(err_msg)
    cli.send_error('[Darknet] ' + err_msg)
    return False
...
def recognize(cli, request, savepath):
  ...
  file_ext = path[-4:]
  gotcha_msg = f'Got a file {path}.' if lang != 'ru_RU' else f'Получен файл {path}.'
  if verbose:
    print(gotcha_msg)
...
def main():
  ...
  os.chdir('darknet')
  print(f'Darknet enabled on port {str(extension_port)}.' if lang != 'ru_RU' \

...
SourcesEditor::SourcesEditor(Basis *_basis, QWidget *parent, ModalHandler *mhandler)
  ...
  source_actions_btn = new Button(tr("Source actions..."), this);
  ...
  save_and_close_btn = new Button(tr("Save and close"), this);
  source_actions_btn->setIcon(
...
void SourcesEditor::connector() {
  ...
  connect(create_source, &QAction::triggered, this, &SourcesEditor::open_create_source);
  connect(remove_source, &QAction::triggered, this, &SourcesEditor::remove);
  connect(save_and_close_btn, &Button::clicked, this, &SourcesEditor::save_and_close);
}
...
void SourcesEditor::save_and_close() {
  if (edited) {
    Sources sources;
...
void SourcesEditor::open_create_source() {
  disconnect(create_source, &QAction::triggered, this, &SourcesEditor::open_create_source);
  ...
  connect(create_source_dialog, &CreateSourceDialog::cancelled, this, &SourcesEditor::close_create_source);
}

/*! @brief Closes the source creation dialog. */
void SourcesEditor::close_create_source() {
  disconnect(create_source_dialog, &CreateSourceDialog::add, this, &SourcesEditor::appendSingle);
  disconnect(create_source_dialog, &CreateSourceDialog::cancelled, this, &SourcesEditor::close_create_source);
